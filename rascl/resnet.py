...
# Changing to Model class to better enable Keras API
class BlockGroup(tf.keras.layers.Layer):  # pylint: disable=missing-docstring
...
# Changing to Model class to better enable Keras API -TW 10/13/22
class Resnet(tf.keras.Model):  # pylint: disable=missing-docstring
  ...
  def __init__(self,
    ...
    if cifar_stem:
      self.initial_conv_relu_max_pool.append(
          Conv2dFixedPadding(
              filters=FILTER_BASE * width_multiplier,
              kernel_size=3,
              strides=1,
              data_format=data_format,
              trainable=trainable))
      ...
      if FLAGS.sk_ratio > 0:  # Use ResNet-D (https://arxiv.org/abs/1812.01187)
        self.initial_conv_relu_max_pool.append(
            Conv2dFixedPadding(
                filters=FILTER_BASE * width_multiplier // 2,
                kernel_size=3,
                strides=2,
                data_format=data_format,
                trainable=trainable))
    ...
    self.block_groups.append(
        BlockGroup(
            filters=FILTER_BASE * 2 * width_multiplier,
            block_fn=block_fn,
            blocks=layers[1],
            strides=2,
            name='block_group2',
    ...
    self.block_groups.append(
        BlockGroup(
            filters=FILTER_BASE * 4 * width_multiplier,
            block_fn=block_fn,
            blocks=layers[2],
            strides=2,
            name='block_group3',
    ...
    self.block_groups.append(
        BlockGroup(
            filters=FILTER_BASE * 8 * width_multiplier,
            block_fn=block_fn,
            blocks=layers[3],
            strides=2,
            name='block_group4',

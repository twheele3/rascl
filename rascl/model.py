...
class ProjectionHead(tf.keras.models.Model):

  def __init__(self, name='projection_head', **kwargs):
  ...
  def call(self, inputs, training):
    ...
    if FLAGS.train_mode == 'finetune':
      hidden_out = FLAGS.ft_proj_selector
    else:
      hidden_out = -1
    return tf.identity(hiddens_list[hidden_out], 'proj_head_output')
    # /NEW
    # proj_head_output = tf.identity(hiddens_list[-1], 'proj_head_output')
    # return proj_head_output, hiddens_list[FLAGS.ft_proj_selector]


class SupervisedHead(tf.keras.models.Model):
...
class Model(tf.keras.models.Model):
  ...
  def call(self, inputs, training):
    ...
    # TODO (TW): move blur preprocessing out of model 
    num_transforms = inputs.shape[3] // FLAGS.num_channels
    num_transforms = tf.repeat(FLAGS.num_channels, num_transforms)
    ...
    #     hiddens, training)
    # NEW (TW 102322)
    outputs = self._projection_head(
        hiddens, training)
    
    if self.train_mode == 'finetune':
      outputs = self.supervised_head(outputs, training)
    return outputs
    # /NEW

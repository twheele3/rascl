...
flags.DEFINE_integer(
    'width_multiplier', 16,
    'Multiplier to change width of network. Base width = 4, ResNet default = 16*4.')
...
flags.DEFINE_integer(
    'rascl_slice_max', 2,
    'If rascl_pretrain=True, the maximum difference between contrastive slices.')
...
flags.DEFINE_integer(
    'kfold_groups', 0,
    'The number of k-fold groups in training data. Does not engage in k-fold training if set to 0. Otherwise runs k-fold cross-validation with assumption that train data has been split and labeled according to {train_split}{kfold_groups} format.') 
...
flags.DEFINE_boolean(
    'pretrain_kfolds', True,
    'If true, during finetuning takes checkpoint as a base directory to find kfold checkpoints.')
...
def try_restore_from_checkpoint(model, global_step, optimizer, kfold):
  ...
  if FLAGS.kfold_groups > 0:
    model_dir = os.path.join(FLAGS.model_dir,f'kfold{kfold}')
  else:
  ...
  if latest_ckpt:
    ...
    if FLAGS.pretrain_kfolds:
      ckpt = os.path.join(FLAGS.checkpoint,f'kfold{kfold}')
      ckpt = os.path.join(ckpt,[i for i in os.listdir(ckpt) if 'index' in i][0])
    else:
      ckpt = FLAGS.checkpoint
    # Restore model weights only, but not global step and optimizer states
    logging.info('Restoring from given checkpoint: %s', ckpt)
    checkpoint_manager2 = tf.train.CheckpointManager(
        tf.train.Checkpoint(model=model),
        directory=model_dir,
        max_to_keep=FLAGS.keep_checkpoint_max)
    checkpoint_manager2.checkpoint.restore(ckpt).expect_partial()
...
def perform_evaluation(model, builder, eval_steps, ckpt, strategy, topology):
  ...
  def single_step(features, labels):
    ...
    supervised_head_outputs = model(features, training=False) # NEW (102322) updating model output
...
def main(argv):
  ...
  # if FLAGS.train_mode == 'pretrain': #Adding Kfold pretraining 15022023
  #   kfolds = 1
  if FLAGS.kfold_groups > 0:
  ...
  for kfold in range(kfolds):
    ...
    if FLAGS.kfold_groups > 0:
      ...
      for k in k_list:
        num_train_examples += builder.info.splits[f'{FLAGS.train_split}{k}'].num_examples
      num_eval_examples = builder.info.splits[f'{FLAGS.train_split}{kfold}'].num_examples
    else:
      num_train_examples = builder.info.splits[FLAGS.train_split].num_examples
      num_eval_examples = builder.info.splits[FLAGS.eval_split].num_examples
    # NEW (102122) Adding custom input pipeline. Scaling to total examples since so few.
    ...
    if FLAGS.mode == 'eval':
      ...
      with strategy.scope():
        ...
        if FLAGS.train_mode == 'pretrain':
          contrast_loss_metric = tf.keras.metrics.Mean('contrast_loss')
          contrast_loss_metric_val = tf.keras.metrics.Mean('contrast_loss')
          contrast_acc_metric = tf.keras.metrics.Mean('contrast_acc')
          contrast_acc_metric_val = tf.keras.metrics.Mean('contrast_acc')
          contrast_entropy_metric = tf.keras.metrics.Mean('contrast_entropy')
          contrast_entropy_metric_val = tf.keras.metrics.Mean('contrast_entropy')
          all_metrics.extend([
          ...
          val_metrics.extend([
            contrast_loss_metric_val, 
            contrast_acc_metric_val, 
            contrast_entropy_metric_val
          ])
        ...
        checkpoint_manager = try_restore_from_checkpoint(
            model, optimizer.iterations, optimizer, kfold)
      ...
      def single_step(features, labels):
        with tf.GradientTape() as tape:
          ...
          with tf.summary.record_if(should_record):
            ...
            tf.summary.image(
                'image', features[:, :, :, :FLAGS.num_channels], step=optimizer.iterations + 1)
          # projection_head_outputs, supervised_head_outputs = model(features, training=True)
          outputs = model(features, training=True) # NEW (102322)
          loss = None
      ...
      def val_single_step(features, labels):
        ...
        if FLAGS.train_mode == 'pretrain':
          ...
          with tf.summary.record_if(True):
            ...
            tf.summary.image(
                'image', features[:, :, :, :FLAGS.num_channels], step=optimizer.iterations + 1)
          # projection_head_outputs, _ = model(features, training=False)
          # outputs = projection_head_outputs
          outputs = model(features, training=False) # NEW (102322) updating output
          con_loss, logits_con, labels_con = obj_lib.add_contrastive_loss(
              outputs,
              hidden_norm=FLAGS.hidden_norm,
              temperature=FLAGS.temperature,
          ...
          if loss is None:
            ...
            loss += con_loss
          metrics.update_pretrain_metrics_val(contrast_loss_metric_val,
                                              contrast_acc_metric_val,
                                              contrast_entropy_metric_val,
                                              con_loss, logits_con,
                                              labels_con)
        if FLAGS.train_mode == 'finetune' or FLAGS.lineareval_while_pretraining:
              # _, supervised_head_outputs = model(features, training=False) 
              # outputs = supervised_head_outputs
              outputs = model(features, training=False) # NEW (102322)
              l = labels['labels']
              sup_loss = obj_lib.add_supervised_loss(labels=l, logits=outputs)
              if loss is None:
                loss = sup_loss
  ...
  if FLAGS.postprocess:
    if FLAGS.train_mode == 'pretrain':
      if FLAGS.kfold_groups > 0:
        for kfold in range(FLAGS.kfold_groups):
          model_dir = os.path.join(FLAGS.model_dir,f'kfold{kfold}')
          path = os.path.join(FLAGS.model_dir,f'val{kfold}')
          ...
          ckpt_best = int(y[:,0][testing.norm(y[:,1]/(x[:,1]*np.log(y[:,0]))).argmax()])
          rmlist = [i for i in os.listdir(model_dir) if 'ckpt' in i and f'ckpt-{ckpt_best}.' not in i]
          for i in rmlist:
            file = os.path.join(model_dir, i)
            if os.path.exists(file):
              os.remove(file)          
      else:
        ckpt_best = testing.pretrain_get_best(FLAGS.model_dir)
        rmlist = [i for i in os.listdir(FLAGS.model_dir) if 'ckpt' in i and f'ckpt-{ckpt_best}.' not in i]
        checkpoint_manager.checkpoint.restore(os.path.join(FLAGS.model_dir, f'ckpt-{ckpt_best}.index')).expect_partial()
        model = checkpoint_manager.checkpoint.model
        model.save(os.path.join(FLAGS.model_dir, f'saved_model-best'))
        for i in rmlist:
          file = os.path.join(FLAGS.model_dir, i)
          if os.path.exists(file):
            os.remove(file)
      
    if FLAGS.train_mode == 'finetune':
      model_ext = [i for i in FLAGS.model_dir.split('/') if len(i)>0][-1]
      images,labels,_ = testing.load_dataset(FLAGS.data_dir, split = 'test')
      with open(os.path.join(FLAGS.data_dir,"labels.txt"), "r") as f:
          labelnames = [i for i in f.read().split("\n") if len(i) > 0]
      y_predicted = testing.run_best_kfold(FLAGS.model_dir, images, labels)
